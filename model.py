...
class SSRNetGeneral(Model):
	def __init__(self, image_size,stage_num,lambda_local,lambda_d, name):
		...
		pred = self.classifier_block()

		super().__init__(inputs=inputs, outputs=pred, name='SSR_Net_'+self.name)
	...
	def classifier_block(self):
		...
		feat_s1_pre = Multiply()([s_layer4,x_layer4])
		delta_s1 = Dense(1,activation='tanh',name=self.name+'_delta_s1')(feat_s1_pre)
		
		feat_s1 = Multiply()([s_layer4_mix,x_layer4_mix])
		feat_s1 = Dense(2*self.stage_num[0],activation='relu')(feat_s1)
		pred_s1 = Dense(units=self.stage_num[0], activation="relu",name=self.name+'_pred_stage1')(feat_s1)
		local_s1 = Dense(units=self.stage_num[0], activation='tanh', name=self.name+'_local_delta_stage1')(feat_s1)
		#-------------------------------------------------------------------------------------------------------------------------
		s_layer2 = Conv2D(10,(1,1),activation='relu')(self.s_layer2)
		s_layer2 = MaxPooling2D(4,4)(s_layer2)
		s_layer2 = Flatten()(s_layer2)
		...
		feat_s2_pre = Multiply()([s_layer2,x_layer2])
		delta_s2 = Dense(1,activation='tanh',name=self.name+'_delta_s2')(feat_s2_pre)
		
		feat_s2 = Multiply()([s_layer2_mix,x_layer2_mix])
		feat_s2 = Dense(2*self.stage_num[1],activation='relu')(feat_s2)
		pred_s2 = Dense(units=self.stage_num[1], activation="relu",name=self.name+'_pred_stage2')(feat_s2)
		local_s2 = Dense(units=self.stage_num[1], activation='tanh', name=self.name+'_local_delta_stage2')(feat_s2)
		#-------------------------------------------------------------------------------------------------------------------------
		s_layer1 = Conv2D(10,(1,1),activation='relu')(self.s_layer1)
		s_layer1 = MaxPooling2D(8,8)(s_layer1)
		s_layer1 = Flatten()(s_layer1)
		...
		feat_s3_pre = Multiply()([s_layer1,x_layer1])
		delta_s3 = Dense(1,activation='tanh',name=self.name+'_delta_s3')(feat_s3_pre)
		
		feat_s3 = Multiply()([s_layer1_mix,x_layer1_mix])
		feat_s3 = Dense(2*self.stage_num[2],activation='relu')(feat_s3)
		pred_s3 = Dense(units=self.stage_num[2], activation="relu",name=self.name+'_pred_stage3')(feat_s3)
		local_s3 = Dense(units=self.stage_num[2], activation='tanh', name=self.name+'_local_delta_stage3')(feat_s3)
		#-------------------------------------------------------------------------------------------------------------------------
		...
		pred = Lambda(SSR_module, 
						arguments={'s1':self.stage_num[0],
								   ...
								   'lambda_d':self.lambda_d,
								   'V':self.V},
								   name=self.name+'_prediction')([pred_s1,pred_s2,pred_s3,delta_s1,delta_s2,delta_s3, local_s1, local_s2, local_s3])
		return pred
	...
	def decodePrediction(self, prediction):
		if self.V == 101 :
			age_predicted = prediction.squeeze()
			return age_predicted
		else :
			gender_predicted = np.around(prediction).astype('int').squeeze()
